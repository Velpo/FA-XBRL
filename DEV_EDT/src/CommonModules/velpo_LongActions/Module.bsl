...
Procedure CancelJobExecution(Val JobID) Export 
	...
	Except
		...
		WriteLogEvent(NStr("en = 'Long actions.Background job cancellation'",  velpo_CommonFunctionsClientServer.DefaultLanguageCode()),
...
Function JobCompleted(Val JobID) Export
	...
	If Job = Undefined Then
		WriteLogEvent(NStr("ru = 'Длительная операция. Фоновое задание не найдено'; en = 'Long actions.Background job not found'", velpo_CommonFunctionsClientServer.DefaultLanguageCode()),
	...
	Else
		...
		ElsIf Job.State =  BackgroundJobState.Canceled Then
			WriteLogEvent(
				NStr("en = 'Long actions.Administrator canceled background job'",  velpo_CommonFunctionsClientServer.DefaultLanguageCode()),
				EventLogLevel.Error,
				,
				,
...
Procedure RegisterProgress(Val Percent =  Undefined, Val Text = Undefined, Val AdditionalParameters = Undefined) Export
	...
	TextToPass = velpo_CommonFunctions.ValueToXMLString(ValueToPass);
	
	Message = New  UserMessage;
	Message.Text = "{LongActions}"  + TextToPass;
	...
	For Number = 1 to Count Do
		...
		If Left(Message.Text, 1) = "{" Then
			...
			If Position >  2 Then
				...
				If MechanismID = "LongActions" Then
					ResultText = Mid(Message.Text, Position + 1);
					Result = velpo_CommonFunctions.ValueFromXMLString(ResultText);
					Break;
				EndIf;
			EndIf;
		EndIf;
