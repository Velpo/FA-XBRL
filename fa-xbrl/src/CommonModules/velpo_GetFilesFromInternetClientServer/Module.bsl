...
Function SplitURL(Val URL) Export
	...
	URLStructure = velpo_CommonFunctionsClientServer.URIStructure(URL);
...
Function URLStructure(Val URLString) Export
	...
	Return velpo_CommonFunctionsClientServer.URIStructure(URLString);
...
Function GetFileFromInternet(Val URL, Val SavingSettings, Val ConnectionSettings = Undefined,
	...
	If Port = Undefined Then
		FullURLStructure = velpo_CommonFunctionsClientServer.URIStructure(URL);
	...
	If FTPProtocolIsUsed Then
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorMessage = NStr("en = 'Error creating FTP connection to the %1 server:'; ru = 'Error creating FTP connection to the %1 server:'") + Chars.LF + "%2";
			
			WriteErrorToEventLog(velpo_StringFunctionsClientServer.SubstituteParametersInString(
				ErrorMessage, ServerName, DetailErrorDescription(ErrorInfo)));
			ErrorMessage = velpo_StringFunctionsClientServer.SubstituteParametersInString(ErrorMessage, ServerName,
	...
	Else
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorMessage = NStr("en = 'Error creating HTTP connection to the %1 server:'; ru = 'Error creating HTTP connection to the %1 server:'") + Chars.LF + "%2";
			WriteErrorToEventLog(
				velpo_StringFunctionsClientServer.SubstituteParametersInString(ErrorMessage, ServerName, 
	...
	Try
		...
		Else
			...
			If HTTPResponse.StatusCode < 200 Or HTTPResponse.StatusCode >= 300 Then
				...
				Raise velpo_StringFunctionsClientServer.ExtractTextFromHTML(ResponseFile.Read(5 * 1024));
			EndIf;
	...
	Except
		ErrorInfo = ErrorInfo();
		ErrorMessage = NStr("en = 'Error downloading file from the %1 server:'; ru = 'Error downloading file from the %1 server:'") + Chars.LF + "%2";
		If WriteError Then
			WriteErrorToEventLog(
				velpo_StringFunctionsClientServer.SubstituteParametersInString(ErrorMessage, ServerName, 
				DetailErrorDescription(ErrorInfo)));
...
Function EventLogMessageText() Export
	...
	Return NStr("en = 'Get files from the Internet'; ru = 'Get files from the Internet'", velpo_CommonFunctionsClientServer.DefaultLanguageCode());
